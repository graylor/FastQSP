  ...
  if (!isFirst && qspCurActionsCount) {
    for (i = 0; i < qspCurActionsCount; ++i) {
      if (qspCurActions[i].Image)
        free(qspCurActions[i].Image);
      free(qspCurActions[i].Desc);
      qspFreePrepLines(qspCurActions[i].OnPressLines,
                       qspCurActions[i].OnPressLinesCount);
    }
...
void qspAddAction(QSPVariant *args, int count, QSPLineOfCode *code, int start,
                  int end, QSP_BOOL isManageLines) {
  QSPCurAct *act;
  QSP_CHAR *imgPath;
  if (qspActIndex(QSP_STR(args[0])) >= 0)
    return;
...
QSP_CHAR *qspGetAllActionsAsCode() {
  ...
  for (i = 0; i < qspCurActionsCount; ++i) {
    ...
    if (qspCurActions[i].Image) {
      ...
      temp = qspReplaceText(qspCurActions[i].Image + qspQstPathLen,
                            QSP_FMT("'"), QSP_FMT("''"));
      len = qspAddText(&res, temp, len, -1, QSP_FALSE);
      free(temp);
    }
    ...
    if (count == 1 && qspIsAnyString(qspCurActions[i].OnPressLines->Str))
      len = qspAddText(&res, qspCurActions[i].OnPressLines->Str, len, -1,
                       QSP_FALSE);
    else {
      if (count >= 2) {
        len = qspAddText(&res, QSP_STRSDELIM, len, QSP_LEN(QSP_STRSDELIM),
                         QSP_FALSE);
        temp = qspJoinPrepLines(qspCurActions[i].OnPressLines, count,
                                QSP_STRSDELIM);
        len = qspAddText(&res, temp, len, -1, QSP_FALSE);
        free(temp);
      }
      len = qspAddText(&res, QSP_STRSDELIM QSP_FMT("END"), len,
                       QSP_LEN(QSP_STRSDELIM) + 3, QSP_FALSE);
    }
    len =
        qspAddText(&res, QSP_STRSDELIM, len, QSP_LEN(QSP_STRSDELIM), QSP_FALSE);
  }
  return res;
}
...
void qspStatementMultilineAddAct(QSPLineOfCode *s, int endLine, int lineInd,
...
QSP_BOOL qspStatementDelAct(QSPVariant *args, int count, QSP_CHAR **jumpTo,
  ...
  if (actInd < 0)
    return QSP_FALSE;
  if (qspCurSelAction >= actInd)
    qspCurSelAction = -1;
  if (qspCurActions[actInd].Image)
    free(qspCurActions[actInd].Image);
  free(qspCurActions[actInd].Desc);
  qspFreePrepLines(qspCurActions[actInd].OnPressLines,
                   qspCurActions[actInd].OnPressLinesCount);
  --qspCurActionsCount;
