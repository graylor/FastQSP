...
void qspCallDebug(QSP_CHAR *str) {
  ...
  if (qspCallBacks[QSP_CALL_DEBUG].IsSet) {
    ...
    AS3_Call(qspCallBacks[QSP_CALL_DEBUG].FuncVal,
             qspCallBacks[QSP_CALL_DEBUG].ThisVal, args);
    AS3_Release(args);
    flyield();
...
void qspCallSetTimer(int msecs) {
  ...
  if (qspCallBacks[QSP_CALL_SETTIMER].IsSet) {
    ...
    AS3_Call(qspCallBacks[QSP_CALL_SETTIMER].FuncVal,
             qspCallBacks[QSP_CALL_SETTIMER].ThisVal, args);
    AS3_Release(args);
    flyield();
    qspRestoreCallState(&state);
  }
...
void qspCallRefreshInt(QSP_BOOL isRedraw) {
  ...
  if (qspCallBacks[QSP_CALL_REFRESHINT].IsSet) {
    ...
    AS3_Call(qspCallBacks[QSP_CALL_REFRESHINT].FuncVal,
             qspCallBacks[QSP_CALL_REFRESHINT].ThisVal, args);
    AS3_Release(args);
    flyield();
    qspRestoreCallState(&state);
  }
...
void qspCallSetInputStrText(QSP_CHAR *text) {
  ...
  if (qspCallBacks[QSP_CALL_SETINPUTSTRTEXT].IsSet) {
    ...
    if (text) {
      ...
      args = AS3_Array("StrType", 0);
    AS3_Call(qspCallBacks[QSP_CALL_SETINPUTSTRTEXT].FuncVal,
             qspCallBacks[QSP_CALL_SETINPUTSTRTEXT].ThisVal, args);
    AS3_Release(args);
    flyield();
    qspRestoreCallState(&state);
  }
...
void qspCallAddMenuItem(QSP_CHAR *name, QSP_CHAR *imgPath) {
  ...
  if (qspCallBacks[QSP_CALL_ADDMENUITEM].IsSet) {
    ...
    if (imgUTF8)
      free(imgUTF8);
    AS3_Call(qspCallBacks[QSP_CALL_ADDMENUITEM].FuncVal,
             qspCallBacks[QSP_CALL_ADDMENUITEM].ThisVal, args);
    AS3_Release(args);
    flyield();
    qspRestoreCallState(&state);
  }
...
void qspCallSystem(QSP_CHAR *cmd) {
  ...
  if (qspCallBacks[QSP_CALL_SYSTEM].IsSet) {
    ...
    if (cmd) {
      ...
      args = AS3_Array("StrType", 0);
    AS3_Call(qspCallBacks[QSP_CALL_SYSTEM].FuncVal,
             qspCallBacks[QSP_CALL_SYSTEM].ThisVal, args);
    AS3_Release(args);
    flyield();
    qspRestoreCallState(&state);
  }
...
void qspCallOpenGame(QSP_CHAR *file) {
  ...
  if (qspCallBacks[QSP_CALL_OPENGAMESTATUS].IsSet) {
    ...
    if (file) {
      ...
      args = AS3_Array("StrType", 0);
    AS3_Call(qspCallBacks[QSP_CALL_OPENGAMESTATUS].FuncVal,
             qspCallBacks[QSP_CALL_OPENGAMESTATUS].ThisVal, args);
    AS3_Release(args);
    flyield();
    qspRestoreCallState(&state);
  }
...
void qspCallSaveGame(QSP_CHAR *file) {
  ...
  if (qspCallBacks[QSP_CALL_SAVEGAMESTATUS].IsSet) {
    ...
    if (file) {
      ...
      args = AS3_Array("StrType", 0);
    AS3_Call(qspCallBacks[QSP_CALL_SAVEGAMESTATUS].FuncVal,
             qspCallBacks[QSP_CALL_SAVEGAMESTATUS].ThisVal, args);
    AS3_Release(args);
    flyield();
    qspRestoreCallState(&state);
  }
...
void qspCallShowMessage(QSP_CHAR *text) {
  ...
  if (qspCallBacks[QSP_CALL_SHOWMSGSTR].IsSet) {
    ...
    if (text) {
      ...
      args = AS3_Array("StrType", 0);
    AS3_Call(qspCallBacks[QSP_CALL_SHOWMSGSTR].FuncVal,
             qspCallBacks[QSP_CALL_SHOWMSGSTR].ThisVal, args);
    AS3_Release(args);
    flyield();
    qspRestoreCallState(&state);
  }
...
int qspCallShowMenu() {
  ...
  if (qspCallBacks[QSP_CALL_SHOWMENU].IsSet) {
    qspSaveCallState(&state, QSP_FALSE, QSP_TRUE);
    args = AS3_Array("");
    AS3_Call(qspCallBacks[QSP_CALL_SHOWMENU].FuncVal,
             qspCallBacks[QSP_CALL_SHOWMENU].ThisVal, args);
    AS3_Release(args);
    flyield();
...
void qspCallShowPicture(QSP_CHAR *file) {
  ...
  if (qspCallBacks[QSP_CALL_SHOWIMAGE].IsSet) {
    ...
    if (file) {
      ...
      args = AS3_Array("StrType", 0);
    AS3_Call(qspCallBacks[QSP_CALL_SHOWIMAGE].FuncVal,
             qspCallBacks[QSP_CALL_SHOWIMAGE].ThisVal, args);
    AS3_Release(args);
    flyield();
    qspRestoreCallState(&state);
  }
...
void qspCallShowWindow(int type, QSP_BOOL isShow) {
  ...
  if (qspCallBacks[QSP_CALL_SHOWWINDOW].IsSet) {
    ...
    AS3_Call(qspCallBacks[QSP_CALL_SHOWWINDOW].FuncVal,
             qspCallBacks[QSP_CALL_SHOWWINDOW].ThisVal, args);
    AS3_Release(args);
    flyield();
    qspRestoreCallState(&state);
  }
...
void qspCallPlayFile(QSP_CHAR *file, int volume) {
  ...
  if (qspCallBacks[QSP_CALL_PLAYFILE].IsSet) {
    ...
    AS3_Call(qspCallBacks[QSP_CALL_PLAYFILE].FuncVal,
             qspCallBacks[QSP_CALL_PLAYFILE].ThisVal, args);
    AS3_Release(args);
    flyield();
    qspRestoreCallState(&state);
  }
...
QSP_BOOL qspCallIsPlayingFile(QSP_CHAR *file) {
  ...
  if (qspCallBacks[QSP_CALL_ISPLAYINGFILE].IsSet) {
    ...
    if (file) {
      ...
      args = AS3_Array("StrType", 0);
    AS3_Call(qspCallBacks[QSP_CALL_ISPLAYINGFILE].FuncVal,
             qspCallBacks[QSP_CALL_ISPLAYINGFILE].ThisVal, args);
    AS3_Release(args);
    flyield();
...
void qspCallSleep(int msecs) {
  ...
  if (qspCallBacks[QSP_CALL_SLEEP].IsSet) {
    qspSaveCallState(&state, QSP_TRUE, QSP_FALSE);
    args = AS3_Array("IntType", msecs);
    AS3_Call(qspCallBacks[QSP_CALL_SLEEP].FuncVal,
             qspCallBacks[QSP_CALL_SLEEP].ThisVal, args);
    AS3_Release(args);
    flyield();
    qspRestoreCallState(&state);
  }
...
int qspCallGetMSCount() {
  /* Здесь получаем количество миллисекунд, прошедших с момента последнего
   * вызова функции */
  QSPCallState state;
  int count;
  AS3_Val args;
  if (qspCallBacks[QSP_CALL_GETMSCOUNT].IsSet) {
    qspSaveCallState(&state, QSP_TRUE, QSP_FALSE);
    args = AS3_Array("");
    AS3_Call(qspCallBacks[QSP_CALL_GETMSCOUNT].FuncVal,
             qspCallBacks[QSP_CALL_GETMSCOUNT].ThisVal, args);
    AS3_Release(args);
    flyield();
...
void qspCallCloseFile(QSP_CHAR *file) {
  ...
  if (qspCallBacks[QSP_CALL_CLOSEFILE].IsSet) {
    ...
    if (file) {
      ...
      args = AS3_Array("StrType", 0);
    AS3_Call(qspCallBacks[QSP_CALL_CLOSEFILE].FuncVal,
             qspCallBacks[QSP_CALL_CLOSEFILE].ThisVal, args);
    AS3_Release(args);
    flyield();
    qspRestoreCallState(&state);
  }
...
void qspCallDeleteMenu() {
  ...
  if (qspCallBacks[QSP_CALL_DELETEMENU].IsSet) {
    qspSaveCallState(&state, QSP_TRUE, QSP_FALSE);
    args = AS3_Array("");
    AS3_Call(qspCallBacks[QSP_CALL_DELETEMENU].FuncVal,
             qspCallBacks[QSP_CALL_DELETEMENU].ThisVal, args);
    AS3_Release(args);
    flyield();
    qspRestoreCallState(&state);
  }
...
QSP_CHAR *qspCallInputBox(QSP_CHAR *text) {
  ...
  if (qspCallBacks[QSP_CALL_INPUTBOX].IsSet) {
    ...
    if (text) {
      ...
      args = AS3_Array("StrType", 0);
    AS3_Call(qspCallBacks[QSP_CALL_INPUTBOX].FuncVal,
             qspCallBacks[QSP_CALL_INPUTBOX].ThisVal, args);
    AS3_Release(args);
    flyield();
