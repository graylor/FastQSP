...
AS3_Val QSPGetCurStateData(void *param, AS3_Val args) {
  ...
  if (qspRealCurLoc >= 0 && qspRealCurLoc < qspLocsCount) {
    ...
    res = AS3_Object("loc:StrType, actIndex:IntType, line:IntType", locUTF8,
                     qspRealActIndex, qspRealLine);
    free(locUTF8);
  } else
    res = AS3_Object("loc:StrType, actIndex:IntType, line:IntType", 0,
                     qspRealActIndex, qspRealLine);
  return res;
}
/* ------------------------------------------------------------ */
...
AS3_Val QSPGetExprValue(void *param, AS3_Val args) {
  ...
  if (v.IsStr) {
    ...
    free(QSP_STR(v));
    res = AS3_Object("isString:IntType, numVal:IntType, strVal:StrType",
                     QSP_TRUE, 0, strUTF8);
    free(strUTF8);
  } else
    res = AS3_Object("isString:IntType, numVal:IntType, strVal:StrType",
                     QSP_FALSE, QSP_NUM(v), 0);
  return res;
}
/* ------------------------------------------------------------ */
/* Текст строки ввода */
...
AS3_Val QSPGetActionData(void *param, AS3_Val args) {
  ...
  if (ind >= 0 && ind < qspCurActionsCount) {
    imageUTF8 =
        (qspCurActions[ind].Image ? qspW2C(qspCurActions[ind].Image) : 0);
...
AS3_Val QSPGetObjectData(void *param, AS3_Val args) {
  ...
  if (ind >= 0 && ind < qspCurObjectsCount) {
    imageUTF8 =
        (qspCurObjects[ind].Image ? qspW2C(qspCurObjects[ind].Image) : 0);
...
AS3_Val QSPGetVarValues(void *param, AS3_Val args) {
  ...
  if (var->Values[ind].Str) {
    ...
    res = AS3_Object("numVal:IntType, strVal:StrType", var->Values[ind].Num,
                     strUTF8);
    free(strUTF8);
  } else
...
AS3_Val QSPGetLastErrorData(void *param, AS3_Val args) {
  ...
  if (qspErrorLoc >= 0 && qspErrorLoc < qspLocsCount) {
    ...
    res = AS3_Object("errorNum:IntType, errorLoc:StrType, "
                     "errorActIndex:IntType, errorLine:IntType",
...
AS3_Val QSPLoadGameWorldFromData(void *param, AS3_Val args) {
  ...
  char *fileName;
  QSP_CHAR *fileNameWC;
  AS3_ArrayValue(args, "AS3ValType, IntType, StrType", &data, &dataSize,
                 &fileName);
  if (qspIsExitOnError && qspErrorNum)
    return AS3_False();
  qspResetError();
...
AS3_Val QSPOpenSavedGameFromData(void *param, AS3_Val args) {
  ...
  QSP_BOOL isRefresh;
  QSP_CHAR *ptr;
  AS3_ArrayValue(args, "AS3ValType, IntType, IntType", &data, &dataSize,
                 &isRefresh);
  if (qspIsExitOnError && qspErrorNum)
    return AS3_False();
  qspPrepareExecution();
...
AS3_Val QSPSetCallBack(void *param, AS3_Val args) {
  ...
  AS3_ArrayValue(args, "IntType, AS3ValType, AS3ValType", &type, &thisVal,
                 &funcVal);
...
int main() {
  ...
  AS3_Val openSavedGameFromData =
      AS3_FunctionAsync(0, QSPOpenSavedGameFromData);
  ...
  AS3_Val result = AS3_Object(
      "QSPIsInCallBack:AS3ValType, QSPEnableDebugMode:AS3ValType, "
      "QSPGetCurStateData:AS3ValType, QSPGetVersion:AS3ValType, "
      "QSPGetCompiledDateTime:AS3ValType, QSPGetFullRefreshCount:AS3ValType, "
      "QSPGetQstFullPath:AS3ValType, "
      "QSPGetCurLoc:AS3ValType, QSPGetMainDesc:AS3ValType, "
      "QSPIsMainDescChanged:AS3ValType, "
      "QSPGetVarsDesc:AS3ValType, QSPIsVarsDescChanged:AS3ValType, "
      "QSPGetExprValue:AS3ValType, "
      "QSPSetInputStrText:AS3ValType, QSPGetActionsCount:AS3ValType, "
      "QSPGetActionData:AS3ValType, "
      "QSPExecuteSelActionCode:AS3ValType, QSPSetSelActionIndex:AS3ValType, "
      "QSPGetSelActionIndex:AS3ValType, "
      "QSPIsActionsChanged:AS3ValType, QSPGetObjectsCount:AS3ValType, "
      "QSPGetObjectData:AS3ValType, "
      "QSPSetSelObjectIndex:AS3ValType, QSPGetSelObjectIndex:AS3ValType, "
      "QSPIsObjectsChanged:AS3ValType, "
      "QSPShowWindow:AS3ValType, QSPGetVarValuesCount:AS3ValType, "
      "QSPGetVarValues:AS3ValType, "
      "QSPGetMaxVarsCount:AS3ValType, QSPGetVarNameByIndex:AS3ValType, "
      "QSPExecString:AS3ValType, "
      "QSPExecLocationCode:AS3ValType, QSPExecCounter:AS3ValType, "
      "QSPExecUserInput:AS3ValType, "
      "QSPGetLastErrorData:AS3ValType, QSPGetErrorDesc:AS3ValType, "
      "QSPLoadGameWorld:AS3ValType, "
      "QSPLoadGameWorldFromData:AS3ValType, QSPSaveGame:AS3ValType, "
      "QSPSaveGameAsData:AS3ValType, "
      "QSPOpenSavedGame:AS3ValType, QSPOpenSavedGameFromData:AS3ValType, "
      "QSPRestartGame:AS3ValType, "
      "QSPSetCallBack:AS3ValType, QSPInit:AS3ValType, QSPDeInit:AS3ValType, "
      "QSPReturnValue:AS3ValType",
      isInCallBack, enableDebugMode, getCurStateData, getVersion,
      getCompiledDateTime, getFullRefreshCount, getQstFullPath, getCurLoc,
      getMainDesc, isMainDescChanged, getVarsDesc, isVarsDescChanged,
      getExprValue, setInputStrText, getActionsCount, getActionData,
      executeSelActionCode, setSelActionIndex, getSelActionIndex,
      isActionsChanged, getObjectsCount, getObjectData, setSelObjectIndex,
      getSelObjectIndex, isObjectsChanged, showWindow, getVarValuesCount,
      getVarValues, getMaxVarsCount, getVarNameByIndex, execString,
      execLocationCode, execCounter, execUserInput, getLastErrorData,
      getErrorDesc, loadGameWorld, loadGameWorldFromData, saveGame,
      saveGameAsData, openSavedGame, openSavedGameFromData, restartGame,
      setCallBack, init, deInit, returnValue);
