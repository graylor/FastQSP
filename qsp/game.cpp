...
int qspQstPathLen = 0;
QSP_CHAR *qspQstFullPath = nullptr;
int qspQstCRC = 0;
...
void qspNewGame(QSP_BOOL isReset) {
  ...
  qspCurLoc = 0;
  if (isReset) {
    qspSetSeed((unsigned int)QSP_TIME(nullptr));
    qspTimerInterval = QSP_DEFTIMERINTERVAL;
    qspCurIsShowObjs = qspCurIsShowActs = qspCurIsShowVars = qspCurIsShowInput =
        QSP_TRUE;
...
static QSP_BOOL qspCheckQuest(char **strs, int count, QSP_BOOL isUCS2) {
  ...
  if (ind > count)
    return QSP_FALSE;
  buf = (isOldFormat ? qspGameToQSPString(strs[0], isUCS2, QSP_FALSE)
                     : qspGameToQSPString(strs[3], isUCS2, QSP_TRUE));
  locsCount = qspStrToNum(buf, nullptr);
  free(buf);
  if (locsCount <= 0)
    return QSP_FALSE;
  for (i = 0; i < locsCount; ++i) {
    ...
    if (isOldFormat)
      ...
      actsCount = qspStrToNum(buf, nullptr);
      free(buf);
      if (actsCount < 0 || actsCount > QSP_MAXACTIONS)
        return QSP_FALSE;
    }
    if ((ind += (actsCount * (isOldFormat ? 2 : 3))) > count)
      return QSP_FALSE;
  }
  return QSP_TRUE;
}

void qspOpenQuestFromData(char *data, int dataSize, QSP_CHAR *fileName,
                          QSP_BOOL isAddLocs) {
  ...
  for (i = start; i < end; ++i) {
    ...
    if (isAddLoc = !isAddLocs || qspLocIndex(buf) < 0)
      ...
      free(buf);
    if (isAddLoc) {
      qspLocs[locsCount].Desc =
          qspGameToQSPString(strs[ind++], isUCS2, QSP_TRUE);
      buf = qspGameToQSPString(strs[ind++], isUCS2, QSP_TRUE);
      qspLocs[locsCount].OnVisitLinesCount =
          qspPreprocessData(buf, &qspLocs[locsCount].OnVisitLines);
      free(buf);
    } else
      ind += 2;
    if (isOldFormat)
    ...
    if (isAddLoc) {
      for (j = 0; j < actsCount; ++j) {
        qspLocs[locsCount].Actions[j].Image =
            (isOldFormat ? nullptr
                         : qspGameToQSPString(strs[ind++], isUCS2, QSP_TRUE));
        qspLocs[locsCount].Actions[j].Desc =
            qspGameToQSPString(strs[ind++], isUCS2, QSP_TRUE);
        buf = qspGameToQSPString(strs[ind++], isUCS2, QSP_TRUE);
        qspLocs[locsCount].Actions[j].OnPressLinesCount =
            qspPreprocessData(buf, &qspLocs[locsCount].Actions[j].OnPressLines);
        free(buf);
      }
      ++locsCount;
    } else
  ...
  if (isAddLocs)
    ...
    delim = qspInStrRChars(qspQstFullPath, QSP_PATHDELIMS, nullptr);
...
int qspSaveGameStatusToString(QSP_CHAR **buf) {
  ...
  if (qspErrorNum)
    return 0;
  QspCodeBuilder builder;
  // *buf = nullptr;
  qspRefreshPlayList();
  locName = (qspCurLoc >= 0 ? qspLocs[qspCurLoc].Name : nullptr);
  builder.WriteVal(QSP_SAVEDGAMEID, QSP_FALSE);
  builder.WriteVal(QSP_VER, QSP_FALSE);
  builder.WriteIntVal(qspQstCRC, QSP_TRUE);
  builder.WriteIntVal(qspGetTime(), QSP_TRUE);
  builder.WriteIntVal(qspCurSelAction, QSP_TRUE);
  builder.WriteIntVal(qspCurSelObject, QSP_TRUE);
  builder.WriteVal(qspViewPath, QSP_TRUE);
  builder.WriteVal(qspCurInput, QSP_TRUE);
  builder.WriteVal(qspCurDesc, QSP_TRUE);
  builder.WriteVal(qspCurVars, QSP_TRUE);
  builder.WriteVal(locName, QSP_TRUE);
  builder.WriteIntVal((int)qspCurIsShowActs, QSP_TRUE);
  builder.WriteIntVal((int)qspCurIsShowObjs, QSP_TRUE);
  builder.WriteIntVal((int)qspCurIsShowVars, QSP_TRUE);
  builder.WriteIntVal((int)qspCurIsShowInput, QSP_TRUE);
  builder.WriteIntVal(qspTimerInterval, QSP_TRUE);
  builder.WriteIntVal(qspPLFilesCount, QSP_TRUE);
  for (i = 0; i < qspPLFilesCount; ++i)
    builder.WriteVal(qspPLFiles[i], QSP_TRUE);
  builder.WriteIntVal(qspCurIncFilesCount, QSP_TRUE);
  for (i = 0; i < qspCurIncFilesCount; ++i)
    builder.WriteVal(qspCurIncFiles[i], QSP_TRUE);
  builder.WriteIntVal(qspCurActionsCount, QSP_TRUE);
  for (i = 0; i < qspCurActionsCount; ++i) {
    if (qspCurActions[i].Image)
      builder.WriteVal(qspCurActions[i].Image + qspQstPathLen,
                            QSP_TRUE);
    else
      builder.WriteVal(nullptr, QSP_FALSE);
    builder.WriteVal(qspCurActions[i].Desc, QSP_TRUE);
    builder.WriteIntVal(qspCurActions[i].OnPressLinesCount,
                             QSP_TRUE);
    for (j = 0; j < qspCurActions[i].OnPressLinesCount; ++j) {
      builder.WriteVal(qspCurActions[i].OnPressLines[j].Str,
                            QSP_TRUE);
      builder.WriteIntVal(
          qspCurActions[i].OnPressLines[j].LineNum, QSP_TRUE);
    }
    builder.WriteIntVal(qspCurActions[i].Location, QSP_TRUE);
    builder.WriteIntVal(qspCurActions[i].ActIndex, QSP_TRUE);
    builder.WriteIntVal(qspCurActions[i].StartLine, QSP_TRUE);
    builder.WriteIntVal((int)qspCurActions[i].IsManageLines,
                             QSP_TRUE);
  }
  builder.WriteIntVal(qspCurObjectsCount, QSP_TRUE);
  for (i = 0; i < qspCurObjectsCount; ++i) {
    if (qspCurObjects[i].Image)
      builder.WriteVal(qspCurObjects[i].Image + qspQstPathLen,
                            QSP_TRUE);
    else
      builder.WriteVal(nullptr, QSP_FALSE);
    builder.WriteVal(qspCurObjects[i].Desc, QSP_TRUE);
  }
  builder.WriteIntVal(qspGetVarsCount(), QSP_TRUE);
  for (i = 0; i < QSP_VARSCOUNT; ++i)
    if (qspVars[i].Name) {
      builder.WriteIntVal(i, QSP_TRUE);
      builder.WriteVal(qspVars[i].Name, QSP_TRUE);
      builder.WriteIntVal(qspVars[i].ValsCount, QSP_TRUE);
      for (j = 0; j < qspVars[i].ValsCount; ++j) {
        builder.WriteIntVal(qspVars[i].Values[j].Num, QSP_TRUE);
        builder.WriteVal(qspVars[i].Values[j].Str, QSP_TRUE);
      }
      builder.WriteIntVal(qspVars[i].IndsCount, QSP_TRUE);
      for (j = 0; j < qspVars[i].IndsCount; ++j) {
        builder.WriteIntVal(qspVars[i].Indices[j].Index, QSP_TRUE);
        builder.WriteVal(qspVars[i].Indices[j].Str, QSP_TRUE);
      }
    }
  *buf = builder.CloneBuffer();
...
static QSP_BOOL qspSkipLines(int *index, int totalLinesCount, int linesToSkip) {
  if ((*index += linesToSkip) >= totalLinesCount)
    return QSP_FALSE;
  return QSP_TRUE;
}

static QSP_BOOL qspGetIntValueAndSkipLine(int *value, int *index,
                                          QSP_CHAR **strs,
                                          int totalLinesCount) {
...
static QSP_BOOL qspCheckGameStatus(QSP_CHAR **strs, int strsCount) {
  ...
  if (qspReCodeGetIntVal(strs[15]) < 0)
    return QSP_FALSE; /* qspTimerInterval */
  /* qspPLFilesCount */
  if (!qspGetIntValueAndSkipLine(&count, &ind, strs, strsCount))
    return QSP_FALSE;
  if (count < 0 || count > QSP_MAXPLFILES)
    return QSP_FALSE;
  /* files */
  if (!qspSkipLines(&ind, strsCount, count))
    return QSP_FALSE;
  /* qspCurIncFilesCount */
  if (!qspGetIntValueAndSkipLine(&count, &ind, strs, strsCount))
    return QSP_FALSE;
  if (count < 0 || count > QSP_MAXINCFILES)
    return QSP_FALSE;
  /* includes */
  if (!qspSkipLines(&ind, strsCount, count))
    return QSP_FALSE;
  ...
  if (!qspGetIntValueAndSkipLine(&count, &ind, strs, strsCount))
    return QSP_FALSE;
  if (count < 0 || count > QSP_MAXACTIONS || selAction >= count)
    return QSP_FALSE;
  /* actions */
  for (i = 0; i < count; ++i) {
    /* image + description */
    if (!qspSkipLines(&ind, strsCount, 2))
      return QSP_FALSE;
    /* lines of code count */
    if (!qspGetIntValueAndSkipLine(&linesCount, &ind, strs, strsCount))
      return QSP_FALSE;
    if (linesCount < 0)
      return QSP_FALSE;
    /* lines */
    for (j = 0; j < linesCount; ++j) {
      /* line of code */
      if (!qspSkipLines(&ind, strsCount, 1))
        return QSP_FALSE;
      /* line number */
      if (!qspGetIntValueAndSkipLine(&temp, &ind, strs, strsCount))
        return QSP_FALSE;
      if (temp < 0)
        return QSP_FALSE;
    }
  ...
  if (!qspGetIntValueAndSkipLine(&count, &ind, strs, strsCount))
    return QSP_FALSE;
  if (count < 0 || count > QSP_MAXOBJECTS || selObject >= count)
    return QSP_FALSE;
  /* objects: image + description */
  if (!qspSkipLines(&ind, strsCount, 2 * count))
    return QSP_FALSE;
  /* variables count */
  if (!qspGetIntValueAndSkipLine(&count, &ind, strs, strsCount))
    return QSP_FALSE;
  ...
  for (i = 0; i < count; ++i) {
    ...
    if (!qspGetIntValueAndSkipLine(&temp, &ind, strs, strsCount))
      return QSP_FALSE;
    if (temp <= lastInd || temp >= QSP_VARSCOUNT)
      return QSP_FALSE;
    lastInd = temp;
    /* variable's name */
    if (!qspSkipLines(&ind, strsCount, 1))
    ...
    if (temp < 0)
      return QSP_FALSE;
    /* values */
    if (!qspSkipLines(&ind, strsCount, 2 * temp))
      return QSP_FALSE;
    /* indices count */
    if (!qspGetIntValueAndSkipLine(&temp, &ind, strs, strsCount))
      return QSP_FALSE;
...
void qspOpenGameStatusFromString(QSP_CHAR *str) {
  ...
  if (*strs[7])
    qspCurInputLen = qspStrLen(qspCurInput = qspCodeReCode(strs[7], QSP_FALSE));
  if (*strs[8])
    qspCurDescLen = qspStrLen(qspCurDesc = qspCodeReCode(strs[8], QSP_FALSE));
  if (*strs[9])
    qspCurVarsLen = qspStrLen(qspCurVars = qspCodeReCode(strs[9], QSP_FALSE));
  ...
  for (i = 0; i < qspCurActionsCount; ++i) {
    if (*strs[ind]) {
      ...
      qspCurActions[i].Image = nullptr;
    ++ind;
    qspCurActions[i].Desc = qspCodeReCode(strs[ind++], QSP_FALSE);
    valsCount = qspCurActions[i].OnPressLinesCount =
        qspReCodeGetIntVal(strs[ind++]);
    if (valsCount) {
      qspCurActions[i].OnPressLines =
          (QSPLineOfCode *)malloc(valsCount * sizeof(QSPLineOfCode));
      for (j = 0; j < valsCount; ++j) {
        qspInitLineOfCode(qspCurActions[i].OnPressLines + j,
                          qspCodeReCode(strs[ind++], QSP_FALSE), 0);
        qspCurActions[i].OnPressLines[j].LineNum =
            qspReCodeGetIntVal(strs[ind++]);
      }
    } else
      qspCurActions[i].OnPressLines = nullptr;
  ...
  for (i = 0; i < varsCount; ++i) {
    ...
    if (valsCount) {
      ...
      qspVars[varInd].Values =
          (QSPVarValue *)malloc(valsCount * sizeof(QSPVarValue));
      for (j = 0; j < valsCount; ++j) {
        qspVars[varInd].Values[j].Num = qspReCodeGetIntVal(strs[ind++]);
        qspVars[varInd].Values[j].Str =
            (*strs[ind] ? qspCodeReCode(strs[ind], QSP_FALSE) : nullptr);
        ++ind;
      }
    }
    valsCount = qspReCodeGetIntVal(strs[ind++]);
    if (valsCount) {
      ...
      qspVars[varInd].Indices =
          (QSPVarIndex *)malloc(valsCount * sizeof(QSPVarIndex));
      for (j = 0; j < valsCount; ++j) {
  ...
  qspFreeStrs(strs, count);
  qspIsMainDescChanged = qspIsVarsDescChanged = qspIsObjectsChanged =
...
QSP_BOOL qspStatementOpenQst(QSPVariant *args, int count, QSP_CHAR **jumpTo,
                             int extArg) {
  QSP_CHAR *file;
  switch (extArg) {
  case 0:
...
QSP_BOOL qspStatementOpenGame(QSPVariant *args, int count, QSP_CHAR **jumpTo,
                              int extArg) {
  QSP_CHAR *file;
  if (count && qspIsAnyString(QSP_STR(args[0]))) {
    ...
    qspCallOpenGame(nullptr);
  return QSP_FALSE;
}

QSP_BOOL qspStatementSaveGame(QSPVariant *args, int count, QSP_CHAR **jumpTo,
                              int extArg) {
  QSP_CHAR *file;
  if (count && qspIsAnyString(QSP_STR(args[0]))) {
    file = qspGetAbsFromRelPath(QSP_STR(args[0]));
