...
unsigned char qspRand8[256] = {
    };
...
static void qspRemoveArray(QSP_CHAR *name) {
  QSPVar *var;
  if (!(var = qspVarReferenceWithType(name, QSP_FALSE, nullptr)))
    return;
  qspEmptyVar(var);
}
...
static void qspRemoveArrayItem(QSP_CHAR *name, int index) {
  ...
  if (index < 0 || index >= var->ValsCount)
    return;
  origIndex = index;
  if (var->Values[index].Str)
    free(var->Values[index].Str);
  var->ValsCount--;
  ...
  for (index = 0; index < var->IndsCount; ++index) {
    ...
    if (ind->Index == origIndex) {
      free(ind->Str);
      var->IndsCount--;
      if (index == var->IndsCount)
        break;
      isRemoving = QSP_TRUE;
    }
    if (isRemoving)
      *ind = var->Indices[index + 1];
    if (ind->Index > origIndex)
      ind->Index--;
  }
}
...
QSPVar *qspVarReferenceWithType(QSP_CHAR *name, QSP_BOOL isCreate,
                                QSP_BOOL *isString) {
  QSPVar *var;
  QSP_CHAR *varName = qspDelSpcCanRetSelf(name);
  if (isString)
    *isString = (*varName == QSP_STRCHAR[0]);
...
static int qspGetVarTextIndex(QSPVar *var, QSP_CHAR *str, QSP_BOOL isCreate) {
  ...
  if (n > 0) {
    ind = (QSPVarIndex *)bsearch(uStr, var->Indices, n, sizeof(QSPVarIndex),
                                 qspIndStringCompare);
    if (ind) {
      free(uStr);
      return ind->Index;
  ...
  if (isCreate) {
    ...
    if (n >= var->IndsBufSize) {
      var->IndsBufSize = n + 8;
      var->Indices = (QSPVarIndex *)realloc(
          var->Indices, var->IndsBufSize * sizeof(QSPVarIndex));
    }
    i = n - 1;
...
int qspPrepareLocalVars(QSPVar **vars) {
  ...
  for (i = qspSavedVarsGroupsCount - 1; i >= 0; --i) {
    for (j = qspSavedVarsGroups[i].VarsCount - 1; j >= 0; --j) {
      if (!(var = qspVarReference(qspSavedVarsGroups[i].Vars[j].Name,
                                  QSP_TRUE))) {
        while (--ind >= 0)
...
void qspRestoreLocalVars(QSPVar *savedVars, int varsCount,
                         QSPVarsGroup *savedGroups, int groupsCount) {
  QSPVar *var;
  int i, j, ind;
  if (savedVars) {
    ind = 0;
...
static void qspCopyVar(QSPVar *dest, QSPVar *src, int start, int count) {
  ...
  for (i = 0; i < maxCount; ++i) {
    ...
    dest->Values[i].Str = (str ? qspGetNewText(str, -1) : nullptr);
  }
  dest->IndsBufSize = 0;
  ...
  for (i = 0; i < src->IndsCount; ++i) {
    ...
    if (newInd >= 0 && newInd < maxCount) {
      if (count >= dest->IndsBufSize) {
        ...
        dest->Indices = (QSPVarIndex *)realloc(
            dest->Indices, dest->IndsBufSize * sizeof(QSPVarIndex));
      }
...
int qspArrayPos(QSP_CHAR *varName, QSPVariant *val, int ind,
                QSP_BOOL isRegExp) {
  int num, count;
  QSPVar *var;
  QSP_CHAR *str;
  ...
  while (ind <= count) {
    if (val->IsStr) {
      if (!(ind < count && (str = var->Values[ind].Str)))
        str = QSP_FMT("");
      if (isRegExp) {
        if (qspRegExpStrMatch(regExp, str))
          return ind;
...
QSPVariant qspArrayMinMaxItem(QSP_CHAR *name, QSP_BOOL isMin) {
  ...
  if (curInd < 0)
    return qspGetEmptyVariant(isString);
...
void qspStatementSetVarValue(QSP_CHAR *s) {
  ...
  if (qspRefreshCount != oldRefreshCount || qspErrorNum)
    return;
  if (pos != s && qspIsInList(QSP_ADD QSP_SUB QSP_DIV QSP_MUL, *(pos - 1)))
    --pos;
  ch = *pos;
  *pos = 0;
...
static QSP_CHAR *qspGetVarNameOnly(QSP_CHAR *s) {
  ...
  if (brackPos)
    return qspGetNewText(s, (int)(brackPos - s));
  return qspDelSpc(s);
}
...
void qspStatementLocal(QSP_CHAR *s) {
  ...
  while (1) {
    ...
    if (isVarFound) {
      ...
      if (count >= bufSize) {
        bufSize = count + 4;
        qspSavedVarsGroups[groupInd].Vars = (QSPVar *)realloc(
            qspSavedVarsGroups[groupInd].Vars, bufSize * sizeof(QSPVar));
      }
...
QSP_BOOL qspStatementCopyArr(QSPVariant *args, int count, QSP_CHAR **jumpTo,
                             int extArg) {
  int start, num;
  QSPVar *dest, *src;
  if (!(dest = qspVarReferenceWithType(QSP_STR(args[0]), QSP_TRUE, nullptr)))
    return QSP_FALSE;
  if (!(src = qspVarReferenceWithType(QSP_STR(args[1]), QSP_FALSE, nullptr)))
    return QSP_FALSE;
  if (dest != src) {
...
QSP_BOOL qspStatementKillVar(QSPVariant *args, int count, QSP_CHAR **jumpTo,
                             int extArg) {
  if (count == 1)
