...
QSP_BOOL qspStatementShowMenu(QSPVariant *args, int count, QSP_CHAR **jumpTo,
                              int extArg) {
  QSPVar *var;
  QSPVariant arg;
  int ind, itemsCount, maxItems, len;
  QSP_CHAR *menuLocs[QSP_MAXMENUITEMS], *imgPath, *str, *pos, *pos2;
  if (!(var = qspVarReferenceWithType(QSP_STR(args[0]), QSP_FALSE, nullptr)))
    return QSP_FALSE;
  qspCallDeleteMenu();
  if (count == 1) {
    ind = 0;
  ...
  while (ind < var->ValsCount) {
    if (itemsCount == maxItems)
      break;
    if (!((str = var->Values[ind].Str) && qspIsAnyString(str)))
      break;
    ...
    if (pos = qspInStrRChars(str, QSP_MENUDELIM, pos2)) {
      len = (int)(pos2 - pos) - 1;
      imgPath = (qspIsAnyString(++pos2) ? qspGetAbsFromRelPath(pos2) : nullptr);
    } else {
      pos = pos2;
      len = -1;
      imgPath = nullptr;
  ...
  if (itemsCount) {
    ...
    if (ind >= 0 && ind < itemsCount) {
      arg.IsStr = QSP_FALSE;
      QSP_NUM(arg) = ind + 1;
      qspExecLocByNameWithArgs(menuLocs[ind], &arg, 1, nullptr);
    }
    while (--itemsCount >= 0)
