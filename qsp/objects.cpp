...
static void qspRemoveObject(int index) {
  ...
  if (index < 0 || index >= qspCurObjectsCount)
    return;
  if (qspCurSelObject >= index)
    qspCurSelObject = -1;
  name.IsStr = QSP_TRUE;
  QSP_STR(name) = qspCurObjects[index].Desc;
  if (qspCurObjects[index].Image)
    free(qspCurObjects[index].Image);
  --qspCurObjectsCount;
...
QSP_BOOL qspStatementAddObject(QSPVariant *args, int count, QSP_CHAR **jumpTo,
                               int extArg) {
  QSPObj *obj;
  int i, objInd;
  QSP_CHAR *imgPath;
  if (count == 3) {
    ...
    if (objInd < 0 || objInd > qspCurObjectsCount)
      return QSP_FALSE;
  } else
...
QSP_BOOL qspStatementDelObj(QSPVariant *args, int count, QSP_CHAR **jumpTo,
                            int extArg) {
  switch (extArg) {
  case 0:
...
QSP_BOOL qspStatementUnSelect(QSPVariant *args, int count, QSP_CHAR **jumpTo,
                              int extArg) {
  qspCurSelObject = -1;
  return QSP_FALSE;
}
