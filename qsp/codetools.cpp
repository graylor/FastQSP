...
void qspFreePrepLines(QSPLineOfCode *strs, int count) {
  if (strs) {
    while (--count >= 0) {
      free(strs[count].Str);
      if (strs[count].Label)
        free(strs[count].Label);
      if (strs[count].Stats)
        free(strs[count].Stats);
    }
    free(strs);
  }
}

void qspCopyPrepLines(QSPLineOfCode **dest, QSPLineOfCode *src, int start,
                      int end) {
  QSPLineOfCode *line;
...
static int qspProcessPreformattedStrings(QSP_CHAR *data, QSPLineOfCode **strs) {
  ...
  QSP_BOOL isNewLine;
  QSP_CHAR *str, quot = 0;
  int lineNum = 0, lastLineNum = 0, count = 0, quotsCount = 0, strLen = 0,
      bufSize = 8, strBufSize = 256;
  str = (QSP_CHAR *)malloc(strBufSize * sizeof(QSP_CHAR));
  ret = (QSPLineOfCode *)malloc(bufSize * sizeof(QSPLineOfCode));
  while (*data) {
    isNewLine =
        (qspStrsNComp(data, QSP_STRSDELIM, QSP_LEN(QSP_STRSDELIM)) == 0);
    if (isNewLine)
      ++lineNum;
...
static int qspProcessEOLExtensions(QSPLineOfCode *s, int count,
                                   QSPLineOfCode **strs) {
  QSPLineOfCode *ret;
  QSP_CHAR *str;
  ...
  while (i < count) {
    ...
    if (len >= QSP_LEN(QSP_PREEOLEXT QSP_EOLEXT)) {
      while (!qspStrsComp(str + len - QSP_LEN(QSP_PREEOLEXT QSP_EOLEXT),
                          QSP_PREEOLEXT QSP_EOLEXT)) {
        if (++i == count)
          break;
        len = qspAddText(&str, s[i].Str, len - QSP_LEN(QSP_EOLEXT), -1,
                         QSP_FALSE);
      }
    }
    if (++newCount > bufSize) {
