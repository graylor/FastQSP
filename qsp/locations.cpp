...
static int qspLocsCompare(const void *locName1, const void *locName2) {
  return qspStrsComp(((QSPLocName *)locName1)->Name,
                     ((QSPLocName *)locName2)->Name);
}
...
void qspCreateWorld(int start, int locsCount) {
  ...
  for (i = start; i < qspLocsCount; ++i) {
    ...
    for (j = 0; j < QSP_MAXACTIONS; ++j) {
      if (qspLocs[i].Actions[j].Desc) {
        if (qspLocs[i].Actions[j].Image)
          free(qspLocs[i].Actions[j].Image);
        free(qspLocs[i].Actions[j].Desc);
        qspFreePrepLines(qspLocs[i].Actions[j].OnPressLines,
                         qspLocs[i].Actions[j].OnPressLinesCount);
      }
    }
  }
  if (qspLocsCount != locsCount) {
    ...
    qspLocs =
        (QSPLocation *)realloc(qspLocs, qspLocsCount * sizeof(QSPLocation));
    qspLocsNames =
        (QSPLocName *)realloc(qspLocsNames, qspLocsCount * sizeof(QSPLocName));
  }
  for (i = start; i < qspLocsCount; ++i) {
...
void qspPrepareLocs() {
  ...
  for (i = 0; i < qspLocsCount; ++i) {
    ...
    qspUpperStr(qspLocsNames[i].Name = qspGetAddText(qspLocsNames[i].Name,
                                                     qspLocs[i].Name, 0, -1));
  }
...
int qspLocIndex(QSP_CHAR *name) {
  ...
  loc = (QSPLocName *)bsearch(uName, qspLocsNames, qspLocsCount,
                              sizeof(QSPLocName), qspLocStringCompare);
  free(uName);
  if (loc)
    return loc->Index;
  return -1;
...
void qspExecLocByIndex(int locInd, QSP_BOOL isChangeDesc, QSP_BOOL isNewLoc) {
  ...
  if (isChangeDesc) {
    ...
    if (*str) {
      qspCurDescLen =
          qspAddText(&qspCurDesc, str, qspCurDescLen, -1, QSP_FALSE);
      qspIsMainDescChanged = QSP_TRUE;
    }
    free(str);
  }
  for (i = 0; i < QSP_MAXACTIONS; ++i) {
    ...
    if (str && *str) {
      ...
      count = 1;
    qspAddAction(args, count, loc->Actions[i].OnPressLines, 0,
                 loc->Actions[i].OnPressLinesCount, QSP_TRUE);
    free(QSP_STR(args[0]));
    if (qspErrorNum) {
      qspRealLine = oldLine;
      qspRealActIndex = oldActIndex;
  ...
  if (locInd < qspLocsCount - qspCurIncLocsCount)
    qspExecTopCodeWithLocals(loc->OnVisitLines, loc->OnVisitLinesCount, 1,
                             isNewLoc);
  else {
...
void qspExecLocByVarNameWithArgs(const QSP_CHAR *name, QSPVariant *args, int count) {
  ...
  while (1) {
    ...
    if (ind >= var->ValsCount)
      break;
    if (!((locName = var->Values[ind].Str) && qspIsAnyString(locName)))
      break;
